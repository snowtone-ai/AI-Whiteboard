...
`feat/browser-input-method`に実装済み。経緯は`docs/decisions.md`のD-009〜D-014を参照。
次の実装者は`CLAUDE.md` → `docs/state.md` → `docs/issues.md` → `tasks.md`の順で読み、
`docs/adapter-smoke.md`の手動検証結果を確認する。

## v1 の境界（ChatGPTのみ）

入力欄横のランチャーボタン → ホワイトボードのオーバーレイ表示 → 「送信」で白背景PNGを
入力欄に添付（自動生成テキストは付加しない、画像のみ。D-014） → ユーザー自身がChatGPTの
送信ボタンを押す。**自動送信・AI応答の読み取りは一切行わない**（D-013、ハード制約）。APIキーの保存や
...
`verify`はlint、型検査、17件のテスト、拡張機能ビルド（esbuild）をまとめて実行する。
...
- `apps/extension/src/content/insert/`: ファイル添付 → クリップボードフォールバックの挿入、
  汎用的なアップロード完了検知（`waitForUploadSettle.ts`、単体テスト4件）
- `apps/extension/src/board/`: Excalidrawボード（`chrome-extension://`ページとして独立実行）、
  白背景PNG書き出しのみ（自動生成テキスト要約は D-014 で削除済み）

...
| claude.ai | 2026-08-21 (partial, live authenticated) | Attach mechanism confirmed live with a real backend upload; packaged-extension pass still open | Driven directly against the user's own real, logged-in Chrome via `chrome-devtools-mcp --autoConnect` (see `docs/state.md` round 5). Confirmed: composer (`[data-testid="chat-input"]`), file input (`input[data-testid="file-upload"]`), and the same `DataTransfer`+`change` attach technique used for ChatGPT all work — a real ~3.5MB test image uploaded and its thumbnail `src` became a genuine `/api/.../files/.../preview` backend URL, stronger confirmation than was possible on chatgpt.com. Found and fixed a real defect: `UPLOAD_INDICATOR_SELECTOR` didn't match claude.ai's busy state (`animate-pulse` on the thumbnail `<img>`, not `progress`/`spinner`/`loading`/`cursor-wait`) — fixed and covered by a regression test; deliberately did **not** add `[role="status"]` even though claude.ai's initial skeleton uses it too, since claude.ai keeps unrelated `role="status"` elements in the DOM at rest which would break settle-detection outright. **Not confirmed**: the packaged extension's own content script (isolated world) running against claude.ai — this round used direct DOM script evaluation against the real tab, not a loaded extension; needs `chrome://extensions` → load unpacked → `apps/extension/dist/` in a real profile. |

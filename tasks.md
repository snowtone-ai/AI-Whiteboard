...
`docs/state.md` rounds 5–11). P007 (claude.ai) is `verified` — the user completed a full manual,
...
reverted it entirely (`prepareForOpen` removed from `types.ts`/`gemini.ts`/`mount.ts`). Round 11
then fixed a real live-use bug where sending from the whiteboard attached nothing: the panel was
auto-closing before the still-pending Ctrl+V happened, and — once fixed to require a manual close —
that close click was found to steal focus away from the composer, silently breaking the paste.
Both are fixed (`Board.tsx`, `mount.ts`) and re-verified live. P006 (chatgpt.com) is `review` —
round 10 closed the login-gated gap once the owner's upload limit reset, driving the real packaged
extension via `chrome-devtools-mcp` against the owner's own authenticated session and confirming
the model actually reads the attached image; all three sites now sit at the same remaining gap, an
owner's-own-hands manual pass with the packaged extension, same shape as P007's round 6.

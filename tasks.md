...
`docs/state.md` rounds 5–10). P007 (claude.ai) is `verified` — the user completed a full manual,
...
manual pass to reach `verified`. P006 (chatgpt.com) is `review` — round 10 closed the login-gated
gap once the owner's upload limit reset, driving the real packaged extension via `chrome-devtools-
mcp` against the owner's own authenticated session and confirming the model actually reads the
attached image; all three sites now sit at the same remaining gap, an owner's-own-hands manual
pass with the packaged extension, same shape as P007's round 6.
